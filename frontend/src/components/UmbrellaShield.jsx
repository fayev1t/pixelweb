...
const UmbrellaShield = ({ isActive, weather }) => {
    ...
    const rainParticles = useMemo(() =>
        Array.from({ length: 12 }).map((_, i) => ({
            id: i,
            left: 10 + seededRandom(i + 11) * 80,
            delay: seededRandom(i + 31) * 0.8,
            duration: 0.2 + seededRandom(i + 51) * 0.25,
            offsetX: (seededRandom(i + 71) - 0.5) * 18,
            bounceH: 5 + seededRandom(i + 91) * 8,
        })), []
    );
    ...
    const snowParticles = useMemo(() =>
        Array.from({ length: 8 }).map((_, i) => ({
            id: i,
            startX: 15 + seededRandom(i + 13) * 70,
            delay: seededRandom(i + 33) * 2,
            duration: 0.5 + seededRandom(i + 53) * 0.8,
            direction: seededRandom(i + 73) > 0.5 ? 1 : -1,
            size: 2 + Math.floor(seededRandom(i + 93) * 2),
        })), []
    );

    const dripParticles = useMemo(() =>
        Array.from({ length: 7 }).map((_, i) => ({
            id: i,
            left: 8 + seededRandom(i + 17) * 84,
            delay: seededRandom(i + 37) * 1.1,
            duration: 0.8 + seededRandom(i + 57) * 0.7,
            length: 3 + Math.floor(seededRandom(i + 77) * 5),
        })), []
    );
    ...
    return (
        ...
        >
            <div className="umbrella-canopy-wrap">
                {/*
                ...
                */}
                ...
                <div className="umbrella-highlight-sheen" />
                <div className="umbrella-rim-shadow" />
                {isSnowing && <div className="umbrella-snow-cap" />}
            </div>

            {isRaining && (
                <div className={`umbrella-edge-drips ${isHeavyRain ? 'heavy' : ''}`}>
                    {dripParticles.map((p) => (
                        <span
                            key={p.id}
                            className="umbrella-edge-drip"
                            style={{
                                left: `${p.left}%`,
                                '--delay': `${p.delay}s`,
